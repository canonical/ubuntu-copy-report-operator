...
def test_setup_systemd_unit_writes_service_and_timer_with_proxy_environment(monkeypatch):
    ...
    monkeypatch.setattr(copy_report.systemd, "service_enable", lambda *args, **kwargs: None)
    monkeypatch.setattr(copy_report.systemd, "daemon_reload", lambda *args, **kwargs: None)
    monkeypatch.setattr(copy_report.systemd, "service_restart", lambda *args, **kwargs: None)
...
def test_configure_schedule_writes_timer_and_restarts_timer(monkeypatch):
    ...
    monkeypatch.setattr(
        copy_report.systemd, "service_restart", lambda *args: calls.append(("restart",) + args)
    )
    ...
    assert ("restart", "copy-report.timer") in calls


def test_setup_systemd_unit_raises_when_enable_fails(monkeypatch):
